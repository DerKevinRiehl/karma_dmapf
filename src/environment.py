import numpy as np
from constants import MAPF_CONTROLLER_CENTRALIZED, MAPF_CONTROLLER_DECENTRALIZED_RESPECT, MAPF_CONTROLLER_DECENTRALIZED_NEGOTIATE_EGOISTIC, MAPF_CONTROLLER_DECENTRALIZED_NEGOTIATE_ALTRUISTIC, MAPF_CONTROLLER_DECENTRALIZED_NEGOTIATE_KAMRA
...
class Environment:
    ...
    def handle_agents(self):
        ...
        elif self.settings["mapf_control"]==MAPF_CONTROLLER_DECENTRALIZED_NEGOTIATE_EGOISTIC:
            self.handle_agents_route_planning_decentralized_negotiate(NegotiationStrategy.negotiate_egoistic)
        elif self.settings["mapf_control"]==MAPF_CONTROLLER_DECENTRALIZED_NEGOTIATE_ALTRUISTIC:
            self.handle_agents_route_planning_decentralized_negotiate(NegotiationStrategy.negotiate_altruistic)
    ...
    def handle_agents_route_planning_centralized(self):
        ...
        if len(planning_relevant_agents)>0:
            ...
            planner = Planner_CBS(grid, cbs_params=self.settings["params_cbs"], astar_params=self.settings["params_astar"])
    ...
    def handle_agents_route_planning_decentralized_negotiate(self, negotiation_function):
        """
        ...
        for agent in planning_relevant_agents:
            ...
            while not planning_finished:
                ...
                conflicts = GridTools.detect_conflicts(current_path, reservation_table=reservation_table_complete)
