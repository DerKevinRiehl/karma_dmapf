...
class Environment:
    ...
    def validate_next_step(self, verbose=True):
        current = {}
        proposed = {}
    
        for agent in self.agents:
            current[agent.id] = tuple(agent.current_position)
    
            # simulate exactly one next action
            x, y = agent.current_position[0], agent.current_position[1]
            theta = agent.current_orientation
    
            if len(agent.route) > 0:
                action = agent.route[0]
        ...
        for pos, ids in pos_to_agents.items():
            if len(ids) > 1:
                if verbose:
                    print(f"[next-step] vertex conflict at {pos}: agents {ids}")
                return True
    
        # edge conflicts
        agent_ids = list(proposed.keys())
        for i in range(len(agent_ids)):
            for j in range(i + 1, len(agent_ids)):
                a = agent_ids[i]
                b = agent_ids[j]
                if proposed[a] == current[b] and proposed[b] == current[a] and proposed[a] != proposed[b]:
                    if verbose:
                        print(f"[next-step] edge conflict: agents {a} and {b}")
                    return True
    
        return False
    
    def get_next_step_proposals(self):
        current = {}
        proposed = {}
    ...
    def detect_next_step_conflicts(self):
        current, proposed = self.get_next_step_proposals()
        conflicts = []
        ...
        for i in range(len(agent_ids)):
            for j in range(i + 1, len(agent_ids)):
                ...
                b = agent_ids[j]
                if (
                    proposed[a] == current[b]
                    and proposed[b] == current[a]
                    and proposed[a] != proposed[b]
                ):
                    conflicts.append({
                        "type": "edge",
                        "agents": [a, b],
                        "edge": (current[a], proposed[a]),
                    })
    
        return conflicts
    
    def repair_immediate_conflicts(self) -> None:
        max_rounds = max(1, len(self.agents) * 2)
        for _ in range(max_rounds):
            conflicts = self.detect_next_step_conflicts()
            # print("\t", "repair_immediate_conflicts_respectful()", len(conflicts), "found")
            if len(conflicts) == 0:
                return
            changed = False
            for conflict in conflicts:
                agent_ids = conflict["agents"]
                # print("\t", "repair_immediate_conflicts_respectful()", "Conflict between agents", agent_ids)
                # for agent_id in agent_ids:
                    # print("\t", "repair_immediate_conflicts_respectful()", 
                          # "Agent", agent_id, "idle", self.get_agent(agent_id).is_idle(), 
                          # "current", self.get_agent(agent_id).current_position, "target", self.get_agent(agent_id).target_position,
                          # "route", self.get_agent(agent_id).route)
                # simple deterministic priority:
                route_a = self.get_agent(agent_ids[0]).route 
                route_b = self.get_agent(agent_ids[1]).route 
                if len(route_a)==0:
                    losing_id = agent_ids[1]
                elif len(route_b)==0:
                    losing_id = agent_ids[0]
                else:
                    # lower id wins, higher id must adapt
                    losing_id = max(agent_ids)
                losing_agent = self.get_agent(losing_id)
                if losing_agent is None:
                    # print("\t", "repair_immediate_conflicts_respectful()", "no looser found")
                    continue
                # print("\t", "repair_immediate_conflicts_respectful()", "Agent", losing_id, "needs to replan")
                # only non-idle agents with a target can be replanned
                if losing_agent.is_idle() or len(losing_agent.target_position) != 2:
                    # print("\t", "repair_immediate_conflicts_respectful()", "not replanned, cause only non-idle agents with a target can be replanned")
                    continue
                # force replanning from current state
                # print("\t", "repair_immediate_conflicts_respectful()", "ok, losers old route", losing_agent.route)
                route_before = losing_agent.route
                losing_agent.route = []
                losing_agent.plan_route_decentralized_respectful()
                route_after = losing_agent.route
                # print("\t", "repair_immediate_conflicts_respectful()", "ok I called replanning()")
                # print("\t", "repair_immediate_conflicts_respectful()", "ok, losers new route", losing_agent.route)
                if route_before==route_after:
                    # print("\t", "repair_immediate_conflicts_respectful()", "hm... nothing changed")
